...
class Updater(training.StandardUpdater):
    ...
    def update_core(self):
        ...
        print('episode: {} iteration: {} R:{} average_R:{}'.format(
              episode, train_iter.iteration, R, average_R))


class GymIterator(chainer.dataset.Iterator):

    def __init__(self, batch_size, replay_start_size):
    ...
    def __next__(self):
        batch = []
        if len(D) >= self.replay_start_size:
            batch_indices = random.sample(range(len(D)), self.batch_size)
            batch = [D[i] for i in sample_indices]
        self.iteration += 1
        return batch
...
def main():
    ...
    train_iter = GymIterator(args.batch_size, args.replay_start_size)
